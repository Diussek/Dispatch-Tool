...
if ($_POST['minus'])
{
    $q = "UPDATE engineers SET incount = incount - 1 WHERE engname='{$_POST['minus']}'";
...
if ($_POST['plus'])
{
    $q = "UPDATE engineers SET incount = incount + 1 WHERE engname='{$_POST['plus']}'";
    $plusresult = mysqli_query($conn, $q);
	$_SESSION[p] = "<b>{$_POST['plus']}</b> got a <b>plus</b> ticket.";
}


$query = "SELECT * FROM engineers WHERE shiftcount=1 && visible=1 ORDER by incount/minioncount";
$result = mysqli_query($conn, $query);

$query1 = "SELECT * FROM engineers WHERE shiftcount=2 && visible=1 ORDER by incount/minioncount";
$result1 = mysqli_query($conn, $query1);

$query2 = "SELECT * FROM engineers WHERE shiftcount=3 && visible=1 ORDER by incount/minioncount";
...
function drawTable($res){
    ...
    while ($row = mysqli_fetch_assoc($res))
    {
        ...
        echo '<tr>
		  ...
		  <td>' . $tickets . '</td>
		  <td>' . $row['incount'] . '</td>
		  <td>' . $row['incount'] / $row['minioncount'] . '</td>
...
$("#send2mailto").on("click", function(e) {
  $.ajax({
      type     : "POST",
      url      : "ajax.inupdate.php",
      data     : $("#dispatch_form").serialize(),
	  dataType : "script",
	  success: function() {   
        window.location='dispatchin.php'  
    }
  });
});
